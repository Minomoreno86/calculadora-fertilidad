...
class EvaluacionFertilidad:
    # ... (El constructor __init__ y todos los otros métodos permanecen sin cambios) ...
    ...
    # ... (resto de métodos sin cambios hasta _evaluar_factor_masculino) ...
    def _evaluar_potencial_por_edad(self):
        ...
        if self.edad < 25: self.diagnostico_potencial_edad, self.probabilidad_base_edad_str, prob_num = "Alta fertilidad", "25-30%", 27.5
            ...
            self.diagnostico_potencial_edad, self.probabilidad_base_edad_str, prob_num = "Probabilidad nula o anecdótica", "0%", 0.0
    ...
    def _evaluar_sop(self):
        if not self.tiene_sop: return; self.recomendaciones_lista.append(RECOMENDACIONES["SOP"]);
    ...
    def _evaluar_factor_masculino(self):
        ...
        alteraciones = []
        # CORRECCIÓN: El factor para Azoospermia ahora es 0.0
        if self.concentracion_esperm == 0:
            alteraciones.append((0.0, "Azoospermia"))
        else:
            if self.concentracion_esperm is not None and self.concentracion_esperm < 15:
    ...
    def ejecutar_evaluacion(self):
        ...
        prob_ajustada_num = (self.probabilidad_base_edad_num * self.imc_factor * self.sop_factor * self.endometriosis_factor * self.mioma_factor * self.adenomiosis_factor * self.polipo_factor * self.hsg_factor *
        ...
        if prob_ajustada_num >= 15:
            self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BUENO", "🟢", f"¡Tu pronóstico de concepción espontánea por ciclo es BUENO ({self.probabilidad_ajustada_final})! Tienes una probabilidad favorable. Se recomienda seguir intentando y mantener un estilo de vida saludable."
        elif prob_ajustada_num >= 5:
            self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "MODERADO", "🟡", f"Tu pronóstico es MODERADO ({self.probabilidad_ajustada_final}). Existen posibilidades, pero hay factores que se pueden optimizar. Considera discutir estos puntos con un especialista."
        else:
            self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BAJO", "🔴", f"Tu pronóstico es BAJO ({self.probabilidad_ajustada_final}). Esto no significa que sea imposible, pero indica que la concepción espontánea puede ser difícil. Se recomienda una evaluación por un especialista en fertilidad."

if __name__ == "__main__":
    # ... (El bloque de prueba de terminal completo va aquí)
    pass