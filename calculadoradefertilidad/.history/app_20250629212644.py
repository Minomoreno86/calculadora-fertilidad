...
# --- Inicialización del Session State (Mantenemos la inicialización completa) ---
...
# --- Botón de Envío y Lógica Principal (SECCIÓN CORREGIDA CON .get()) ---
st.divider()
if st.button("Generar Informe de Fertilidad (Prueba Pasos 1-2)", type="primary", use_container_width=True):
    try:
        # APLICAMOS LECTURA DEFENSIVA CON .get()
        # Para cada clave, proporcionamos un valor por defecto que coincide con la inicialización.
        tiene_miomas_val = (st.session_state.get("miomas_radio", "No") == 'Sí')
        
        evaluacion = EvaluacionFertilidad(
            # Datos Paso 1 (estos siempre existen, no necesitan .get)
            edad=st.session_state.edad,
            duracion_ciclo=st.session_state.duracion_ciclo,
            imc=st.session_state.imc,
            
            # Datos Paso 2 (usamos .get para los condicionales)
