...
st.divider()
if st.button("Generar Informe de Fertilidad (Prueba Pasos 1-3)", type="primary", use_container_width=True):
    try:
        ...
        evaluacion = EvaluacionFertilidad(
            ...
            resultado_hsg=st.session_state.get("resultado_hsg", "normal") if st.session_state.get("hsg_radio", "No") == 'Sí' else "",
            # --- AÑADIMOS DATOS PASO 3 ---
            amh=st.session_state.get("amh", 2.5),
            prolactina=st.session_state.get("prolactina", 15.0),
            tsh=st.session_state.get("tsh", 2.0),
            tpo_ab_positivo=(st.session_state.get("tpo_radio", "No") == 'Sí'),
            insulina_ayunas=st.session_state.get("insulina_ayunas", 8.0),
            glicemia_ayunas=st.session_state.get("glicemia_ayunas", 90.0)
        )
