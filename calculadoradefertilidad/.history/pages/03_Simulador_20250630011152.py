...
# CÓDIGO COMPLETO Y REFACTORIZADO
...
# ####################################################################
# --- 5. Lógica de Recálculo (SECCIÓN REFACTORIZADA) ---
# ####################################################################

# Se mantiene la lógica de validación de datos
...
# Se mantiene la limpieza de datos para el constructor
datos_simulados.pop('id', None)
datos_simulados.pop('timestamp', None)
datos_simulados.pop('pronostico_final', None)

# --- LÍNEAS MODIFICADAS ---
# El código aquí ahora es mucho más limpio y legible.
...
# 3. Obtenemos los resultados directamente, sin manipulaciones de strings.
#    El modelo nos da los datos puros y los datos formateados por separado.
pronostico_simulado_num = evaluacion_simulada.pronostico_numerico
pronostico_simulado_display = evaluacion_simulada.pronostico_display
# --- FIN DE LÍNEAS MODIFICADAS ---

with col2:
    st.metric(
        label="Nuevo Pronóstico Simulado", 
        value=pronostico_simulado_display, # Usamos el valor ya formateado del modelo
