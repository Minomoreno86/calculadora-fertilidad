...
if registro_seleccionado_str:
    ...
    col1, col2 = st.columns([0.8, 1.2]) # Damos un poco más de espacio a la columna de simulación

    with col1:
        st.subheader("🔵 Perfil Original")
        st.metric(label="Pronóstico Original Guardado", value=f"{perfil_base['pronostico_final']:.1f}%")
        
        # --- Mostramos más datos originales ---
    ...
    # Sobrescribimos los valores con los de TODOS los sliders
    ...
    # La llamada a EvaluacionFertilidad ya acepta todos estos parámetros, no hay que cambiarla.
