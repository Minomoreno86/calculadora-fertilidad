...
if 'step' not in st.session_state:
    ...
    # Paso 1
    st.session_state.edad = 30; st.session_state.duracion_ciclo = 28; st.session_state.imc = 0
    # Paso 2
    st.session_state.sop_radio = "No"; st.session_state.endo_radio = "No"; st.session_state.grado_endometriosis = 1; st.session_state.miomas_radio = "No"; st.session_state.mioma_submucoso_radio = "No"; st.session_state.mioma_submucoso_multiple_radio = "No"; st.session_state.mioma_intramural_radio = "No"; st.session_state.mioma_subseroso_radio = "No"; st.session_state.adeno_radio = "No"; st.session_state.tipo_adenomiosis = "focal"; st.session_state.polipo_radio = "No"; st.session_state.tipo_polipo = "pequeno_unico"; st.session_state.hsg_radio = "No"; st.session_state.resultado_hsg = "normal"
    # Paso 3
    st.session_state.use_amh = False; st.session_state.amh = 2.5; st.session_state.use_prolactina = False; st.session_state.prolactina = 15.0; st.session_state.use_tsh = False; st.session_state.tsh = 2.0; st.session_state.tpo_radio = "No"; st.session_state.use_homa = False; st.session_state.insulina_ayunas = 8.0; st.session_state.glicemia_ayunas = 90.0
    # Paso 4
    st.session_state.use_esperma = False; st.session_state.volumen_seminal = 2.5; st.session_state.concentracion_esperm = 40.0; st.session_state.motilidad_progresiva = 45; st.session_state.morfologia_normal = 5; st.session_state.vitalidad_esperm = 75
...
with tab1: ui_perfil_basico()
with tab2: ui_historial_clinico()
with tab3: ui_laboratorio()
with tab4: ui_factor_masculino()
...
if st.button("Generar Informe de Fertilidad Completo", type="primary", use_container_width=True):
    try:
        ...
        evaluacion = EvaluacionFertilidad(
            edad=st.session_state.get("edad", 30), duracion_ciclo=st.session_state.get("duracion_ciclo", 28), imc=st.session_state.get("imc", 0),
        ...
        st.error(f"Ha ocurrido un error inesperado al generar el informe: {e}"); st.error("Detalle técnico: " + str(e));
        if 'evaluacion_actual' in st.session_state: del st.session_state.evaluacion_actual
...
if 'evaluacion_actual' in st.session_state:
    ...
    if st.button("💾 Guardar este Perfil y Resultado", use_container_width=True):
        ...
        if conn is not None:
            try: pronostico_num = float(evaluacion.probabilidad_ajustada_final.replace('%', ''))
            except: pronostico_num = 0.0
            registro = (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), evaluacion.edad, evaluacion.duracion_ciclo, evaluacion.imc, 1 if evaluacion.tiene_sop else 0, evaluacion.grado_endometriosis, 1 if evaluacion.tiene_miomas else 0, 1 if evaluacion.mioma_submucoso else 0, 1 if evaluacion.mioma_intramural_significativo else 0, 1 if evaluacion.mioma_subseroso_grande else 0, evaluacion.amh, evaluacion.prolactina, evaluacion.tsh, 1 if evaluacion.tpo_ab_positivo else 0, evaluacion.insulina_ayunas, evaluacion.glicemia_ayunas, evaluacion.concentracion_esperm, evaluacion.motilidad_progresiva, evaluacion.morfologia_normal, evaluacion.vitalidad_esperm, pronostico_num)
            registro_id = insertar_registro(conn, registro); conn.close()
            if registro_id: st.success(f"¡Éxito! Perfil guardado correctamente con el ID: {registro_id}"); del st.session_state.evaluacion_actual
            else: st.error("Hubo un error al intentar guardar el perfil en la base de datos.")