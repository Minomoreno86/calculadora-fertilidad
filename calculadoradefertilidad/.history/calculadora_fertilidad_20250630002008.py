...
class EvaluacionFertilidad:
    ...
    def ejecutar_evaluacion(self):
        ...
        if prob_ajustada_num >= 15: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BUENO", "🟢", f"¡Tu pronóstico de concepción espontánea por ciclo es BUENO ({self.probabilidad_ajustada_final})! Tienes una probabilidad favorable."
        elif prob_ajustada_num >= 5: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "MODERADO", "🟡", f"Tu pronóstico es MODERADO ({self.probabilidad_ajustada_final}). Existen posibilidades, pero hay factores que se pueden optimizar."
        else: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BAJO", "🔴", f"Tu pronóstico es BAJO ({self.probabilidad_ajustada_final}). Se recomienda encarecidamente una evaluación por un especialista."
...
if __name__ == '__main__':
    ...
    datos_prueba = { "edad": 36, "amh": 0.2 }
    evaluacion_prueba = EvaluacionFertilidad(**datos_prueba)
    evaluacion_prueba.ejecutar_evaluacion()
    print(f"\nResultado de la prueba para edad {datos_prueba['edad']}:")
    print(f"Pronóstico: {evaluacion_prueba.probabilidad_ajustada_final}")
