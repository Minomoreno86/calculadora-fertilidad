# textos_clinicos.py

# Este archivo funciona como un 'almacén' central para todos los textos de la aplicación.
# Facilita la modificación de los textos sin tocar la lógica principal.

COMENTARIOS_DIAGNOSTICOS = {
    "IMC_BAJO": "Bajo peso: Puede alterar el eje hormonal y reducir la frecuencia de ovulación.",
    "IMC_NORMAL": "Peso normal: Contribuye a un entorno hormonal óptimo para la concepción.",
    "IMC_SOBREPESO": "Sobrepeso: Puede asociarse a resistencia a la insulina leve y alterar la regularidad ovulatoria.",
    "IMC_OBESIDAD": "Obesidad: Impacto negativo en la fertilidad por posible anovulación y disminución de la calidad de los óvulos.",
    
    "SOP_LEVE": "SOP Leve: Ciclos relativamente regulares. El impacto en la fertilidad espontánea es menor, pero existente.",
    "SOP_MOD_SEV": "SOP Moderado/Severo: Anovulación crónica y/u obesidad. La fertilidad espontánea está muy reducida.",
    
    # ... y así sucesivamente para cada diagnóstico ...
}

RECOMENDACIONES = {
    "EDAD_35": "Edad >35a: No demorar la consulta especializada si no hay embarazo en 6 meses.",
    "EDAD_38": "Edad >38a: Se recomienda evaluación temprana por especialista en fertilidad.",
    "EDAD_40": "Edad >40a: Valorar opciones de reproducción asistida como vía principal.",
    
    "IMC_BAJO": "IMC: Optimizar nutrición para alcanzar un peso saludable y regularizar la ovulación.",
    "IMC_ALTO": "IMC: Iniciar plan de dieta y ejercicio. Una pérdida del 5% del peso puede mejorar la fertilidad.",
    
    "SOP": "SOP: Consultar sobre inducción de la ovulación (ej. Letrozol) como tratamiento de primera línea.",
    
    "ENDO_LEVE": "Endometriosis Grado I-II: Puede intentarse concepción espontánea/dirigida, valorar tratamiento si no hay éxito.",
    "ENDO_SEVERA": "Endometriosis Grado III-IV: Se recomienda valoración directa para FIV.",

    "MIOMA_SUBMUCOSO": "Mioma Submucoso: Se recomienda resección por histeroscopia para mejorar la implantación.",
    "MIOMA_INTRAMURAL": "Mioma Intramural Significativo: Discutir con su médico la opción de miomectomía según el caso.",
    
    "ADENO_FOCAL": "Adenomiosis Focal: Valorar tratamiento médico para controlar síntomas y mejorar receptividad.",
    "ADENO_DIFUSA": "Adenomiosis Difusa: Se recomienda valoración para FIV, ya que la fertilidad espontánea está muy reducida.",

    "POLIPO": "Pólipo Endometrial: Se recomienda resección por histeroscopia (polipectomía) para optimizar la cavidad endometrial.",

    "HSG_UNILATERAL": "HSG: Obstrucción Unilateral: Considerar seguimiento folicular para dirigir relaciones al lado permeable.",
    "HSG_BILATERAL": "HSG: Obstrucción Bilateral: La concepción espontánea no es viable. Se requiere FIV.",
    "HSG_DEFECTO": "HSG: Defecto Uterino: Se requiere histeroscopia para diagnóstico y tratamiento.",

    "AMH_BAJA": "AMH: Reserva ovárica disminuida. No demorar consulta y tratamiento con especialista.",
    
    "PRL_ALTA": "Prolactina: Nivel elevado. Requiere tratamiento con agonistas dopaminérgicos (ej. Cabergolina) y estudio por endocrinólogo.",
    
    "TSH_ALTA": "TSH: Nivel fuera del rango óptimo para fertilidad (<2.5). Se recomienda tratamiento de sustitución y control por endocrinólogo.",
    "TPO_POSITIVO": "TPO-Ab: Autoinmunidad tiroidea presente. Requiere seguimiento estricto durante el embarazo por riesgo de aborto.",
    
    "HOMA_ALTO": "HOMA: Resistencia a la insulina detectada. Priorizar cambios en estilo de vida y valorar uso de metformina.",
    
    "FACTOR_MASCULINO": "Factor Masculino: Hallazgos alterados en espermatograma. Se recomienda consulta con Andrólogo/Urólogo."
}