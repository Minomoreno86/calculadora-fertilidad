...
# Usamos una clave para el selector para poder detectar cambios
registro_seleccionado_str = st.selectbox("Selecciona tu perfil base:", opciones_registros, key="profile_selector")

# Si el registro seleccionado cambia, reseteamos las variables a simular
if 'registro_id_seleccionado' not in st.session_state or st.session_state.registro_id_seleccionado != registro_seleccionado_str:
    ...
    st.session_state.variables_a_simular = [] # Forzamos el reseteo
...
# Determinamos las variables que tienen un valor en el perfil cargado para pre-seleccionarlas
variables_con_datos = [var for var in SIMULATABLE_VARIABLES if perfil_base.get(var) not in [None, 0, False]]

# Si la lista de simulación en el estado está vacía (por reseteo), la llenamos con las variables con datos
    ...
    for var_name in sorted(st.session_state.variables_a_simular):
        ...
        if info['type'] == 'boolean':
            ...
            if original_value is not None:
                # Usamos el operador % para formateo, que es más seguro que el f-string interno
...
with col2:
    ...
    for var_name in sorted(st.session_state.variables_a_simular):
        ...
        if original_value is not None:
            default_value = float(original_value) if isinstance(original_value, (float, int)) else info['min']
        else:
