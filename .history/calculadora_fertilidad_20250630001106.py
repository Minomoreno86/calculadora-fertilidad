...
class EvaluacionFertilidad:
    ...
    def ejecutar_evaluacion(self):
        ...
        if prob_ajustada_num >= 15: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BUENO", "🟢", f"¡Tu pronóstico de concepción espontánea por ciclo es BUENO ({self.probabilidad_ajustada_final})! Tienes una probabilidad favorable. Se recomienda seguir intentando y mantener un estilo de vida saludable."
        elif prob_ajustada_num >= 5: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "MODERADO", "🟡", f"Tu pronóstico es MODERADO ({self.probabilidad_ajustada_final}). Existen posibilidades, pero hay factores que se pueden optimizar. Considera discutir estos puntos con un especialista."
        else: self.pronostico_categoria, self.pronostico_emoji, self.pronostico_frase = "BAJO", "🔴", f"Tu pronóstico es BAJO ({self.probabilidad_ajustada_final}). Esto no significa que sea imposible, pero indica que la concepción espontánea puede ser difícil. Se recomienda una evaluación por un especialista en fertilidad."
