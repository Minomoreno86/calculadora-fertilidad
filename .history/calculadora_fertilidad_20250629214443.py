...
class EvaluacionFertilidad:
    ...
    def _evaluar_factor_masculino(self):
        ...
        if alteraciones:
            alteracion_principal = min(alteraciones, key=lambda item: item[0]); self.male_factor = alteracion_principal[0]; self.diagnostico_masculino_detallado = ", ".join([item[1] for item in alteraciones]); self.recomendaciones_lista.append(RECOMENDACIONES["FACTOR_MASCULINO"])
