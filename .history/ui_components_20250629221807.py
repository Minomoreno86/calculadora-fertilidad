...
def ui_laboratorio():
    ...
    with st.expander("Expandir para introducir Perfil Endocrino y Metabólico"):
        st.checkbox("Evaluar Nivel de AMH", key="use_amh", value=st.session_state.get("use_amh", False))
        st.number_input("Nivel de AMH (ng/mL)", 0.0, 20.0, format="%.2f", key="amh", disabled=not st.session_state.get("use_amh"))
        st.divider()
        
        st.checkbox("Evaluar Nivel de Prolactina", key="use_prolactina", value=st.session_state.get("use_prolactina", False))
        st.number_input("Nivel de Prolactina (ng/mL)", 0.0, 200.0, format="%.2f", key="prolactina", disabled=not st.session_state.get("use_prolactina"))
        st.divider()
        
        st.checkbox("Evaluar Función Tiroidea (TSH)", key="use_tsh", value=st.session_state.get("use_tsh", False))
        st.number_input("Nivel de TSH (µUI/mL)", 0.0, 10.0, format="%.2f", key="tsh", disabled=not st.session_state.get("use_tsh"))
        ...
        st.checkbox("Evaluar Índice HOMA (Resistencia a Insulina)", key="use_homa", value=st.session_state.get("use_homa", False))
        st.number_input("Insulina en ayunas (μU/mL)", 1.0, 50.0, format="%.2f", key="insulina_ayunas", disabled=not st.session_state.get("use_homa"))
        st.number_input("Glicemia en ayunas (mg/dL)", 50.0, 200.0, format="%.1f", key="glicemia_ayunas", disabled=not st.session_state.get("use_homa"))
    st.divider()

def ui_factor_masculino():
    st.markdown("#### Paso 4 de 4: Factor Masculino")
    st.checkbox("Incluir análisis de espermatograma en la evaluación", key="use_esperma", value=st.session_state.get("use_esperma", False))
    with st.expander("Expandir para introducir Detalle del Espermatograma"):
        is_disabled = not st.session_state.get("use_esperma")
        st.number_input("Volumen (mL)", 0.0, 10.0, format="%.2f", key="volumen_seminal", disabled=is_disabled)
        st.number_input("Concentración (millones/mL)", 0.0, 200.0, format="%.2f", key="concentracion_esperm", disabled=is_disabled)
        st.number_input("Motilidad progresiva (%)", 0, 100, key="motilidad_progresiva", disabled=is_disabled)
        st.number_input("Morfología normal (%)", 0, 100, key="morfologia_normal", disabled=is_disabled)
        st.number_input("Vitalidad (%)", 0, 100, key="vitalidad_esperm", disabled=is_disabled)
    st.divider()
