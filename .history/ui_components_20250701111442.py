# ui_components.py (Versión COMPLETA y ROBUSTA)
...
def ui_perfil_basico():
    ...
    peso = st.number_input("Peso (kg)", min_value=30.0, max_value=200.0, value=st.session_state.get('peso', 65.0), format="%.1f", key="peso")
    talla = st.number_input("Talla (m)", min_value=1.0, max_value=2.5, value=st.session_state.get('talla', 1.65), format="%.2f", key="talla")
...
def ui_historial_clinico():
    ...
    st.radio("¿Diagnóstico de SOP?", ["No", "Sí"], key="sop_radio", index=0 if st.session_state.get("sop_radio") == "No" else 1)
    
    st.radio("¿Diagnóstico de Endometriosis?", ["No", "Sí"], key="endo_radio", index=0 if st.session_state.get("endo_radio") == "No" else 1)
    if st.session_state.get("endo_radio") == 'Sí':
        st.selectbox("Grado de Endometriosis", [1, 2, 3, 4], key="grado_endometriosis")
        
    st.radio("¿Diagnóstico de Miomatosis?", ["No", "Sí"], key="miomas_radio", index=0 if st.session_state.get("miomas_radio") == "No" else 1)
    if st.session_state.get("miomas_radio") == 'Sí':
        st.radio("¿Miomas SUBMUCOSOS?", ["No", "Sí"], key="mioma_submucoso_radio", index=0 if st.session_state.get("mioma_submucoso_radio") == "No" else 1)
        if st.session_state.get("mioma_submucoso_radio") == 'Sí':
            st.radio("¿Son múltiples?", ["No", "Sí"], key="mioma_submucoso_multiple_radio", index=0 if st.session_state.get("mioma_submucoso_multiple_radio") == "No" else 1)
        else:
            st.radio("¿Miomas INTRAMURALES ≥ 4cm o que deformen cavidad?", ["No", "Sí"], key="mioma_intramural_radio", index=0 if st.session_state.get("mioma_intramural_radio") == "No" else 1)
            st.radio("¿Miomas SUBSEROSOS > 6cm?", ["No", "Sí"], key="mioma_subseroso_radio", index=0 if st.session_state.get("mioma_subseroso_radio") == "No" else 1)
            
    st.radio("¿Diagnóstico de Adenomiosis?", ["No", "Sí"], key="adeno_radio", index=0 if st.session_state.get("adeno_radio") == "No" else 1)
    ...
    st.radio("¿Diagnóstico de Pólipos?", ["No", "Sí"], key="polipo_radio", index=0 if st.session_state.get("polipo_radio") == "No" else 1)
    ...
    st.radio("¿Resultado de HSG disponible?", ["No", "Sí"], key="hsg_radio", index=0 if st.session_state.get("hsg_radio") == "No" else 1)
...
def ui_laboratorio():
    ...
    with st.expander("Expandir para introducir Perfil Endocrino y Metabólico", expanded=True):
...
def ui_factor_masculino():
    ...
    with st.expander("Expandir para introducir Detalle del Espermatograma", expanded=True):
        is_disabled = not st.session_state.get("use_esperma", False)
