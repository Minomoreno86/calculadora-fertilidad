...
def crear_tabla(conn):
    ...
    CREATE TABLE IF NOT EXISTS registros (
        ...
        mioma_subseroso_grande INTEGER, tipo_adenomiosis TEXT, tipo_polipo TEXT, resultado_hsg TEXT,
        amh REAL, prolactina REAL, tsh REAL, tpo_ab_positivo INTEGER,
        insulina_ayunas REAL, glicemia_ayunas REAL, volumen_seminal REAL,
...
def insertar_registro(conn, registro_data):
    sql = ''' INSERT INTO registros(timestamp, edad, duracion_ciclo, imc, tiene_sop, 
                                    ...
                                    glicemia_ayunas, volumen_seminal, concentracion_esperm, 
                                    motilidad_progresiva, morfologia_normal, vitalidad_esperm, 
                                    pronostico_final)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
    try:
        cursor = conn.cursor()
