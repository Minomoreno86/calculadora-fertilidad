...
def ui_historial_clinico():
    ...
    with col1:
        ...
        st.toggle("Diagnóstico de SOP", key="tiene_sop")
        ...
        if tiene_polipos:
            st.selectbox(
                ...
                key="tipo_polipo",  # La key correcta para el backend
                format_func=lambda x: {
                    "pequeno_unico": "Pequeño y único",
                    "moderado_multiple": "Moderado o múltiple",
                    "grande": "Grande (>1.5cm)"
                }.get(x, x)
            )

    with col2:
        # --- Miomatosis Uterina (SECCIÓN ACTUALIZADA) ---
        ...
        if tiene_hsg:
            st.selectbox(
                ...
                }.get(x) if x else "Selecciona un resultado"
            )
def ui_laboratorio():
