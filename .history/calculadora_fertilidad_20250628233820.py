class EvaluacionFertilidad:
    def __init__(self, edad, duracion_ciclo=None, imc=None, amh=None, prolactina=None, tsh=None, tpo_ab_positivo=False, tiene_sop=False, grado_endometriosis=0, 
        ...
        # NUEVO: Añadidos 'tsh' y 'tpo_ab_positivo'.
        """
        # --- Factores de Entrada Femeninos ---
        self.edad, self.duracion_ciclo, self.imc, self.amh, self.prolactina, self.tsh, self.tpo_ab_positivo, self.tiene_sop, self.grado_endometriosis = \
            edad, duracion_ciclo, imc, amh, prolactina, tsh, tpo_ab_positivo, tiene_sop, grado_endometriosis
        ...
        # NUEVO: Atributos para TSH
        self.comentario_tsh, self.tsh_factor = "", 1.0
    ...
    def _evaluar_tsh(self):
        """
        # NUEVO: Evalúa la función tiroidea y determina el factor de ajuste.
        """
        if self.tsh is None:
            self.datos_faltantes.append("Nivel de TSH")
            return

        # La regla de TPO-Ab tiene prioridad
        if self.tsh > 2.5 and self.tpo_ab_positivo:
            self.comentario_tsh = "Hipotiroidismo subclínico con autoinmunidad. Aumenta riesgo de fallo de implantación y aborto."
            self.tsh_factor = 0.5
        elif self.tsh > 4.0:
            self.comentario_tsh = "Hipotiroidismo clínico. Reduce significativamente la fertilidad."
            self.tsh_factor = 0.6
        elif self.tsh > 2.5:
            self.comentario_tsh = "Hipotiroidismo subclínico. Puede afectar la ovulación y aumentar el riesgo de aborto."
            self.tsh_factor = 0.85
        else: # TSH <= 2.5
            self.comentario_tsh = "Función tiroidea óptima para la fertilidad."
            self.tsh_factor = 1.0
    ...
    def ejecutar_evaluacion(self):
        ...
        prob_ajustada = self.probabilidad_base_edad_num * self.imc_factor * self.sop_factor * self.endometriosis_factor * self.prolactina_factor * self.tsh_factor * self.male_factor
        self.probabilidad_ajustada_final = f"{prob_ajustada:.1f}%"

# --- FASE DE INTERACCIÓN (con flujo de preguntas mejorado) ---

print("--- Evaluación de Fertilidad de Pareja v10 (con TSH y Mejor UX) ---")
print("\n--- Datos Femeninos ---")
# ... (Inputs femeninos) ...
...
print("\n--- Datos Masculinos ---")
# ACTUALIZADO: Flujo de preguntas para el espermatograma
...
try:
    ...
    # ... (print de factores)
    factores_femeninos = evaluacion.imc_factor * evaluacion.sop_factor * evaluacion.endometriosis_factor
    factores_endocrinos = evaluacion.prolactina_factor * evaluacion.tsh_factor
    print(f"  x Factores Ginecológicos (IMC, SOP, Endo): {factores_femeninos:.2f}")
    print(f"  x Factores Endocrinos (PRL, TSH): {factores_endocrinos:.2f}")
