# 01_Calculadora.py (Versión FINAL y COMPLETA con Lógica de Fiabilidad CORREGIDA)
...
if st.session_state.get('edad') and st.session_state.get('duracion_ciclo') and st.session_state.get('imc'):
    puntos_fiabilidad += 1
if st.session_state.get('hsg_radio') == 'Sí':
    puntos_fiabilidad += 1
if st.session_state.get('use_amh') or st.session_state.get('use_tsh') or st.session_state.get('use_homa') or st.session_state.get('use_prolactina'):
    puntos_fiabilidad += 1
if st.session_state.get('use_esperma'):
    puntos_fiabilidad += 1

puntos_fiabilidad = min(puntos_fiabilidad, max_puntos)
...
if fiabilidad <= 0.25:
    st.info("ℹ️ El pronóstico se basa solo en datos básicos. Considera añadir resultados de HSG, laboratorio y espermatograma.")
elif fiabilidad < 1.0:
     st.warning("⚠️ ¡Buen comienzo! Completa las secciones restantes para obtener la máxima fiabilidad.")
else:
     st.success("✅ ¡Excelente! El pronóstico tiene la máxima fiabilidad.")
