...
import numpy as np
from benchmarks import BENCHMARK_RIESGO_ABORTO

class CalculadoraRiesgoAborto:
    def obtener_benchmark_poblacional(self):
        if self.edad < 30:
            return BENCHMARK_RIESGO_ABORTO["Menos de 30"]
        elif 30 <= self.edad <= 34:
            return BENCHMARK_RIESGO_ABORTO["30-34"]
        elif 35 <= self.edad <= 39:
            return BENCHMARK_RIESGO_ABORTO["35-39"]
        elif 40 <= self.edad <= 44:
            return BENCHMARK_RIESGO_ABORTO["40-44"]
        else:
            return BENCHMARK_RIESGO_ABORTO["45+"]
    ...
    def calcular_riesgo(self):
        ...
        benchmark = self.obtener_benchmark_poblacional()
        diferencia = riesgo_final - benchmark

        if diferencia > 5:
            comparacion = "mayor al promedio poblacional"
        elif diferencia < -5:
            comparacion = "menor al promedio poblacional"
        else:
            comparacion = "similar al promedio poblacional"

        mensaje_benchmark = f"El riesgo basal para tu grupo de edad es {benchmark}%. Tu riesgo calculado ({riesgo_final}%) es {comparacion}."
        ...
        return {
            ...
            "mensaje_benchmark": mensaje_benchmark
        }
