    ...
    try:
        ...
        if not db_existe:
            ...
            if not tabla_logros:
                crear_tabla_logros(conn)
                inicializar_logros(conn)
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='riesgo_aborto'")
            tabla_riesgo = cursor.fetchone()
            if not tabla_riesgo:
                crear_tabla_riesgo_aborto(conn)
        return conn
...
def inicializar_logros(conn):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        conn.commit()
def crear_tabla_riesgo_aborto(conn):
    sql = """
    CREATE TABLE IF NOT EXISTS riesgo_aborto (
        ...
        tiene_miomas INTEGER,
        tiene_adenomiosis INTEGER,
        tiene_sop INTEGER,
        tiene_diabetes INTEGER,
        tiene_tiroides INTEGER,
        prolactina_alta INTEGER,
        infecciones_previas INTEGER,
        calidad_semen_alterada INTEGER,
        riesgo_final REAL,
        categoria TEXT
    );
    """
    try:
        ...
        print(f"Error al crear la tabla riesgo_aborto: {e}")
