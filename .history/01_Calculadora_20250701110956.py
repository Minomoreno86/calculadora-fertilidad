# 01_Calculadora.py (Versión FINAL y COMPLETA con Navegación e Indicador de Fiabilidad)
...
# --- INDICADOR DE FIABILIDAD DEL PRONÓSTICO ---
st.subheader("Fiabilidad del Pronóstico")

secciones_completadas = 1
secciones_totales = 4

if st.session_state.get('sop_radio') == 'Sí' or st.session_state.get('endo_radio') == 'Sí' or st.session_state.get('miomas_radio') == 'Sí':
    secciones_completadas += 1
...
fiabilidad = min(secciones_completadas, secciones_totales) / secciones_totales

st.progress(fiabilidad, text=f"Nivel de Precisión: {int(fiabilidad * 100)}%")
st.caption("Mientras más datos proporciones, más preciso y personalizado será el pronóstico.")

if fiabilidad <= 0.25:
    st.info("ℹ️ El pronóstico se basará solo en datos básicos. Para una evaluación más completa, añade datos de los siguientes pasos.")
elif fiabilidad <= 0.75:
     st.warning("⚠️ ¡Buen comienzo! Añadir todas las secciones te dará el pronóstico más fiable.")
else:
     st.success("✅ ¡Excelente! Con todos los datos proporcionados, el pronóstico es altamente fiable y personalizado.")
